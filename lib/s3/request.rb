...
module S3
  ...
  class Request < Struct.new(:host, :method, :path, :headers, :body, :content_type)
    CREDENTIALS = YAML.load_file(File.join(Rails.root, 'config', 'amazon_s3.yml'))
    ...
    def setup_headers(options = {})
      ...
      self.headers["authorization"] = ::S3::Signature.generate(:host => self.host,
                                                    :request => self,
                                                    :access_key_id => CREDENTIALS[Rails.env]['access_key_id'],
                                                    :secret_access_key => CREDENTIALS[Rails.env]['secret_access_key'])
    end
    ...
    def run
      ...
      if Rails.env == 'test' && !response.mock
